...
public class BaseActivity extends AppCompatActivity implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    protected final String[] permissionsNeededForPhotoTagging = new String[]{Manifest.permission.CAMERA,
    ...
    protected void requestRunTimePermissions(final Activity activity, final String[] permissions,
        ...
        if (permissions.length == 1) {
            if(customPermissionConstant==MY_PERMISSIONS_REQUEST_READ_CONTACTS || customPermissionConstant==MY_PERMISSION_REQUEST_WRITE_CONTACTS) {
            ...
            else if(customPermissionConstant==EXTERNAL_STORAGE_PERMISSION)
            {
                if (ActivityCompat.shouldShowRequestPermissionRationale(this, permissions[0])) {

                    Snackbar.make(findViewById(android.R.id.content), "App fdfd permission to work", Snackbar.LENGTH_INDEFINITE).setAction("ENABLE",
                            new View.OnClickListener() {
                                @Override
                                public void onClick(View v) {
                                    ActivityCompat.requestPermissions(activity, permissions, customPermissionConstant);
